...
class PrePostCalculator {
    ...
    fun calculate(input: TDMInput.PrePost): TDMResult {
        ...
        steps += CalculationStep("Pre-dose Level (Cpre)", "$cPre mg/L", "Entered trough level")
        steps += CalculationStep("Post-dose Level (Cpost)", "$cPost mg/L", "Entered peak level")
        steps += CalculationStep("Pre-to-Post Gap (deltaT)", "$deltaT hr", "Time gap between Cpost and Cpre")
        ...
        if (deltaT <= 0.0) {
            throw IllegalArgumentException(
                "Time gap between pre and post samples ($deltaT hr) must be > 0."
            )
        }

        val tPost = tinf + hoursAfterInfusionEnd
        val tPre = tPost + deltaT
        val hoursBeforeNextDose = max(0.0, tau - tPre)

        if (tPre <= tPost) {
            throw IllegalArgumentException(
                "Pre-sample time (tPre) must be greater than post-sample time (tPost)."
            )
        }

        val ke = ln(cPost / cPre) / deltaT
        ...
        val cMax = cPost * exp(ke * hoursAfterInfusionEnd)
        steps += CalculationStep(
        ...
        val cMin = cPre * exp(-ke * hoursBeforeNextDose)
        steps += CalculationStep(
        ...
        val k0 = dose / tinf
        val vdNumerator = k0 * (1.0 - exp(-ke * tinf))
        val vdDenominator = ke * (cMax - cMin * exp(-ke * tinf))

        if (vdDenominator <= 0.0) {
            throw IllegalArgumentException(
                "Calculated volume of distribution is non-positive due to invalid concentration values."
            )
        }

        val vd = vdNumerator / vdDenominator
        steps += CalculationStep(
